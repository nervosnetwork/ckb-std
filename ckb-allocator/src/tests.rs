use crate::block_list_alloc::{BlockListAllocInner as BlockListAlloc, BLOCK_SIZE};

fn with_allocator<F: FnOnce(BlockListAlloc)>(f: F, buf: &[u8]) {
    let allocator = unsafe {
        let addr = buf.as_ptr();
        let len = buf.len();
        BlockListAlloc::new(addr, len)
    };
    f(allocator);
}
