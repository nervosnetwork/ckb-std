...
fn test_exec_by_index() {
    ...
    let caller_bin = {
        let mut buf = Vec::new();
        File::open("../target/riscv64imac-unknown-none-elf/debug/examples/exec_caller")
            .unwrap()
            .read_to_end(&mut buf)
            .expect("read code");
        Bytes::from(buf)
    ...
    let callee_bin = {
        let mut buf = Vec::new();
        File::open("../target/riscv64imac-unknown-none-elf/debug/examples/exec_callee")
            .unwrap()
            .read_to_end(&mut buf)
            .expect("read code");
        Bytes::from(buf)
...
fn test_exec_by_code_hash() {
    ...
    let caller_bin = {
        let mut buf = Vec::new();
        File::open("../target/riscv64imac-unknown-none-elf/debug/examples/exec_caller_by_code_hash")
            .unwrap()
            .read_to_end(&mut buf)
            .expect("read code");
        Bytes::from(buf)
