...
#[test]
fn test_spawn_by_index() {
    let mut context = Context::default();
    let caller_bin = {
        let mut buf = Vec::new();
        File::open("../build/debug/spawn-caller")
            .unwrap()
            .read_to_end(&mut buf)
            .expect("read code");
        Bytes::from(buf)
    ...
    let callee_bin = {
        let mut buf = Vec::new();
        File::open("../build/debug/spawn-callee")
            .unwrap()
            .read_to_end(&mut buf)
            .expect("read code");
        Bytes::from(buf)
...
#[test]
fn test_spawn_by_code_hash() {
    let mut context = Context::default();
    let caller_bin = {
        let mut buf = Vec::new();
        File::open("../build/debug/spawn-caller-by-code-hash")
            .unwrap()
            .read_to_end(&mut buf)
            .expect("read code");
        Bytes::from(buf)
